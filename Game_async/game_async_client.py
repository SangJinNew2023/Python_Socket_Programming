...
async def conn_handler(run, host="localhost"):
    ...
    pos = await reader.read(2048) # 자신의 플레이어 객체를 수신
    pos1 = pickle.loads(pos)
    clock = pygame.time.Clock()

    while run:
        clock.tick(60)
        writer.write(pickle.dumps(pos1)) #자신의 플레이어 객체를 전송
        await writer.drain()
        pos2 = await reader.read(2048) #상대방 플레이어 객체를 수신

        #윈도우 종료 버튼(x)를 누르면 게임을 종료한다.
